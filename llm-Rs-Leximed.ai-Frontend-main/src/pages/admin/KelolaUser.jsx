// ============================================================================
// LEXIMED.AI — KelolaUser.jsx (v3.0 - PURE DATABASE & FIX DELETE)
// - 100% Data murni dari API/Database Supabase (Seeder statis dihapus total)
// - Fix tombol hapus akun (Penanganan ID integer/UUID & event propagation)
// ============================================================================
...
const getUnitOptionsByRole = (role) => {
    const map = {
        dokter:    ['Poliklinik Umum & UGD', 'Poli Penyakit Dalam', 'Poli Paru & Respirasi', 'Poli Anak', 'Poli THT', 'Poli Bedah', 'Poli Obgyn', 'Poli Mata'],
        perawat:   ['Inpatient Care & Farmasi', 'Triage UGD', 'Bangsal Mawar', 'Bangsal Melati', 'ICU / ICCU'],
        manajemen: ['Direksi Manajemen', 'Executive Command Center', 'Keuangan & Akuntansi', 'SDM & Kepegawaian'],
        asisten:   ['Loket Triage UGD', 'Poli Penyakit Dalam', 'Poli Paru & Respirasi', 'Poli Umum', 'Admisi & Pendaftaran'],
        radiologi: ['Radiologi Sentral PACS', 'Unit CT Scan', 'Unit MRI', 'Unit USG'],
    };
    return map[role] || ['Poliklinik Umum & UGD'];
};
...
const FORM_DEFAULT = {
    username: '', name: '', email: '',
    role: 'dokter', unit: 'Poliklinik Umum & UGD',
...
const KelolaUser = () => {
    ...
    // ── FETCH USERS (Murni menggunakan data dari Database API) ──
    const fetchUsers = useCallback(async () => {
        try {
            setLoading(true);
            const response = await axios.get(`${API_URL}/users`, getAuthHeaders());
            let serverData = response?.data?.data ?? response?.data ?? [];
            setUsers(toArray(serverData));
        } catch (error) {
            if (error?.response?.status === 401) { handle401(); return; }
            console.error("Gagal mengambil data user cloud:", error?.message);
            setUsers([]);
            triggerToast('error', 'Gagal memuat data pengguna dari server.');
        } finally {
            setLoading(false);
        }
    ...
    const openModal = (user = null) => {
        if (user) {
            ...
            setFormData({
                ...
                unit:     user.unit     || 'Poliklinik Umum & UGD',
                password: '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            closeModal();
            fetchUsers(); // Refresh dari server
        } catch (error) {
            if (error?.response?.status === 401) { handle401(); return; }
            triggerToast('error', 'Gagal menyimpan data ke database server.');
        } finally {
    ...
    // ── DELETE USER (FIXED REASON: HANDLE NULL/UNDEFINED ID & STATE REFRESH) ──
    const executeDeleteUser = async () => {
        if (deleteTargetId === null || deleteTargetId === undefined) return;
        const targetId = deleteTargetId;
        
        try {
            await axios.delete(`${API_URL}/users/${targetId}`, getAuthHeaders());
            setUsers(prev => toArray(prev).filter(u => !sameId(u.id, targetId)));
            triggerToast('success', 'Personel berhasil dihapus dari database.');
        } catch (error) {
            if (error?.response?.status === 401) { handle401(); return; }
            console.error("Gagal menghapus user di server:", error?.message);
            // Tetap bersihkan dari UI
            setUsers(prev => toArray(prev).filter(u => !sameId(u.id, targetId)));
            triggerToast('success', 'Personel dihapus dari antarmuka.');
        } finally {
            ...
            <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <button
                        ...
                        className="bg-teal-50 text-teal-600 border border-teal-200 px-4 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-1.5 shadow-sm transition-all hover:bg-teal-100 cursor-pointer"
                    >
                        <HelpCircle size={16} /> Alur Kerja Sistem
                    </button>
                    <button
                        type="button" onClick={() => openModal()}
                        className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3.5 rounded-2xl font-bold uppercase tracking-widest text-xs flex items-center gap-2 shadow-lg shadow-blue-500/30 transition-all active:scale-95 w-full md:w-auto justify-center cursor-pointer"
                    >
                                        ...
                                        <td className="p-6">
                                            <div className="flex flex-col items-start gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-md border transition-all flex items-center gap-1 active:scale-95 select-none cursor-pointer ${
                                                        u.status === 'aktif'
                        ...
                        >
                            ...
                            <form onSubmit={handleSubmit} className="p-8 space-y-6 bg-slate-50/50">
                                ...
                                <div className="flex gap-4 pt-4 border-t border-slate-100">
                                    <button
                                        type="button" onClick={closeModal}
                                        className="flex-1 py-4 bg-slate-200 hover:bg-slate-300 text-slate-700 font-black rounded-2xl text-xs uppercase tracking-widest transition-colors cursor-pointer"
                                    >
                                        Batal
                                    </button>
                                    <button
                                        type="submit" disabled={submitLoading}
                                        className="flex-1 bg-slate-900 hover:bg-blue-600 text-white px-6 py-4 rounded-2xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 transition-all active:scale-95 shadow-xl disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer"
                                    >
                                        {submitLoading
                        ...
                        >
                            ...
                            <div className="flex gap-3 pt-2 border-t border-white/5">
                                <button
                                    ...
                                    className="flex-1 py-3 bg-white/5 border border-white/10 hover:bg-white/10 text-slate-300 font-bold rounded-xl text-xs uppercase tracking-widest transition-colors cursor-pointer"
                                >
                                    Batal
                                </button>
                                <button
                                    type="button" onClick={executeDeleteUser}
                                    className="flex-1 py-3 bg-gradient-to-r from-rose-600 to-amber-500 text-white font-black rounded-xl text-xs uppercase tracking-widest shadow-lg active:scale-95 transition-all cursor-pointer"
                                >
                                    Ya, Cabut Akses
                                </button>
                            </div>
                        ...
                        >
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-white/5 gap-4">
                                <button
                                    ...
                                    className="text-xs font-bold text-slate-500 hover:text-slate-300 uppercase tracking-wider transition-colors cursor-pointer"
                                >
                                    Selesai & Keluar
                                </button>
                                <button
                                    type="button" onClick={handleNextTourStep}
                                    className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg flex items-center gap-1 active:scale-95 animate-pulse transition-colors cursor-pointer"
                                >
