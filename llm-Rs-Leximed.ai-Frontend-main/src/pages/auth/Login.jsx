// ============================================================================
// LEXIMED.AI — Login.jsx (v17.5 - FIXED CASE-SENSITIVE AUTH & SECURE PAYLOAD)
// 100% Menangani Case-Sensitivity PostgreSQL & Payload JSON Backend Laravel
// Mempertahankan 100% Layout Grid, Animasi Framer Motion, Estetika Glow & Tour
// ============================================================================
...
export default function Login() {
  ...
  const tourSteps = [
    ...
    },
    {
      ...
      credential: { user: "admin", note: "Kata sandi: password" }
    },
    {
      ...
      credential: { user: "asisten_tirta", note: "Kata sandi: password" }
    },
    {
      ...
      credential: { user: "radiologi-01", note: "Kata sandi: password" }
    },
    {
      ...
      credential: { user: "dr_tirta", note: "Kata sandi: password" }
    },
    {
      ...
      credential: { user: "manajemen-1", note: "Kata sandi: password" }
    }
  ];
  ...
  const handleLogin = async (e) => {
    ...
    let targetUsername = usernameVal.trim().toLowerCase();
    ...
    if (!targetUsername && !targetPassword) {
      if (role === 'admin') {
        ...
        targetUsername = 'asisten_tirta';
        targetPassword = 'password';
      } else if (role === 'perawat') {
        targetUsername = 'perawat-1';
        targetPassword = 'password';
      } else {
    ...
    try {
      ...
      if (!contentType || !contentType.includes("application/json")) {
        throw new Error(`Layanan Cloud Gateway sedang sibuk (HTTP ${response.status}). Silakan coba klik tombol AUTHENTICATE sekali lagi.`);
      }
      ...
      setError(err.message === "Failed to fetch" 
        ? "Koneksi Cloud Gateway Terputus. Pastikan perangkat Anda terhubung internet, lalu coba lagi." 
        : err.message
      );
      generateCaptcha(); 
      setLoading(false);
        ...
        <div className="p-6 md:p-12 lg:p-20 flex flex-col justify-center bg-[#0f172a]/40 relative">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div className="grid grid-cols-3 gap-2 p-1.5 bg-white/5 rounded-[2.5rem] border border-white/5">
              {roleList.map((r) => (
                <button 
                  ...
                  className={`flex flex-col items-center gap-2 py-5 rounded-2xl transition-all duration-500 relative group overflow-hidden cursor-pointer ${
                    role === r.id 
            ...
            <div className="space-y-4">
              ...
              <div className="relative group">
                ...
                <button 
                  ...
                  className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-200 transition-colors cursor-pointer"
                >
              ...
              <div className="p-4 bg-white/[0.02] border border-white/10 rounded-2xl flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <button 
                    ...
                    className="p-2.5 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl text-slate-400 hover:text-emerald-400 transition-all active:scale-95 shrink-0 cursor-pointer"
                  >
                    <RefreshCw size={14} />
                  </button>
                </div>
              ...
              <button 
                ...
                className="w-full bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 py-3.5 rounded-xl text-xs font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 cursor-pointer"
              >
            ...
            <motion.button 
              ...
              type="submit"
              className={`w-full py-6 rounded-[1.5rem] font-black text-lg tracking-[0.3em] transition-all relative overflow-hidden group cursor-pointer ${
                loginSuccess 
            ...
            >
              ...
              <div className="flex items-center justify-between pt-4 border-t border-white/5">
                <button 
                  ...
                  className="text-xs font-bold text-slate-500 hover:text-white uppercase tracking-wider transition-colors cursor-pointer"
                >
                  Lewati Panduan
                </button>
                ...
                <div className="flex gap-3">
                  {tourStep > 0 && (
                    <button 
                      ...
                      className="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl text-xs font-bold text-white uppercase tracking-wider transition-all cursor-pointer"
                    >
                      Kembali
                    </button>
                  )}
                  ...
                  <button 
                    ...
                    className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 rounded-xl text-xs font-black text-white uppercase tracking-widest flex items-center gap-1 shadow-lg shadow-emerald-900/40 transition-all active:scale-95 cursor-pointer"
                  >
